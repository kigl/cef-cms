<?php
return [
    'Button add to cart' => 'В корзину',
    'Button refresh cart' => 'Пересчитать',
    'Button order' => 'Оформить заказ',

	'Module name' => 'Интеренет магазин',
	'Module description' => 'Интеренет магазин',

    'Checkout' => 'Оформить заказ',
    'Create group' => 'Создание группы',
    'Create product' => 'Создание продукта',
    'Create product property' => 'Создание свойства продукта',

    'Edit group: {data}' => 'Редактирование группы: {data}',
    'Edit product: {data}' => 'Редактирование продукта: {data}',
    'Edit product property: {data}' => 'Редактирование свойства продукта: {data}',

    'Page header cart' => 'Корзина',
	'Product relation' => 'Модификация для товара',
    'Product properties' => 'Свойства продуктов',
    'Property type' => 'Тип свойства',
    'Price: {price, number, currency}' => 'Цена: {price, number, currency}',
    'Products' => 'Продукты',

    'Status not available' => 'Нет в наличии',
    'Sum' => 'Сумма',

    'User id' => 'Идентификатор пользователя',

    'Quick view product' => 'Быстрый просмотр',

    'Toolbar products' => 'Продукты',
    'Toolbar product properties' => 'Свойства продуктов',
    'Toolbar order' => 'Заказы',
    'Tab order view detail' => 'Информация',
    'Tab order view items' => 'Состав заказа',
	'Tab modifications' => 'Модификации',
    'Text  price' => 'Цена: ',

    'Orders' => 'Заказы',
];